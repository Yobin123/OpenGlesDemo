...
public abstract class AFilter implements GLSurfaceView.Renderer {
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        ...
        float sWidthHeight = width / (float) height; //控件宽高比
        uXY = sWidthHeight;
        if (width > height) { //宽度大于高度  横向
            if (sWH > sWidthHeight) {
    ...
    private int createTexture() {
        ...
        if (mBitmap != null && !mBitmap.isRecycled()) {
            ...
            //设置缩小过滤未使用纹理中坐标最接近的一个像素颜色作为绘制的像素颜色。
            GLES20.glTexParameterf(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MIN_FILTER, GLES20.GL_NEAREST);
            //设置放大过滤为使用纹理中坐标中最接近的若干颜色，通过加权平均算法得到需要绘制的像素颜色
            GLES20.glTexParameterf(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MAG_FILTER, GLES20.GL_LINEAR);
            //设置环绕方向s,截取纹理图标（1/2n,1-1/2n）.将导致永远不会与border融合
            GLES20.glTexParameterf(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_WRAP_S, GLES20.GL_CLAMP_TO_EDGE);
            //设置环绕方向T.截取纹理坐标（1/2n,1-1/2n）将导致永远不可闭合
